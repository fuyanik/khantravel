  ...
  return (
    <div className="md:h-screen h-auto w-screen  flex items-center justify-center bg-slate-100 md:pt-24 pt-20"> 
      ...
      <MobileDateTimePicker
        isOpen={showDateTimePicker && isMobile}
        onClose={() => setShowDateTimePicker(false)}
        value={selectedDateTime}
        onChange={(newValue) => handleDateTimeChange(newValue)}
        title="Select Date & Time"
        dateFading={dateDateFading}
        timeFading={dateTimeFading}
      />

      {/* Mobile Pickup Date Time Picker Modal */}
      <MobileDateTimePicker
        isOpen={showPickupDateTimePicker && isMobile}
        onClose={() => setShowPickupDateTimePicker(false)}
        value={selectedPickupDateTime}
        onChange={(newValue) => handlePickupDateTimeChange(newValue)}
        title="Select Pickup Date & Time"
        dateFading={pickupDateFading}
        timeFading={pickupTimeFading}
      />

      {/* Mobile Return Date Time Picker Modal */}
      <MobileDateTimePicker
        isOpen={showReturnDateTimePicker && isMobile}
        onClose={() => setShowReturnDateTimePicker(false)}
        value={selectedReturnDateTime}
        onChange={(newValue) => handleReturnDateTimeChange(newValue)}
        title="Select Return Date & Time"
        dateFading={returnDateFading}
        timeFading={returnTimeFading}
      />
